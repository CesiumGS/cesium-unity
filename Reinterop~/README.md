...
Some resources that may be helpful if you need to go deep on Reinterop:

* [C++ and C# Interop in Cesium for Unity](https://cesium.com/blog/2023/01/26/developing-for-unity/) - A high-level introduction to Reinterop, and some justification for why it exists.
* [C++ Scripting (from Jackson Dunstall])(https://www.jacksondunstan.com/articles/3938) - A long series of articles on writing Unity plugins in C++ by making the Unity C# API callable from C++. This was the inspiration for Reinterop, and had a massive influence on it. There are some key differences, however, outlined in [this issue].(https://github.com/CesiumGS/cesium-unity/issues/1)
* [cesium-unity issue #1](https://github.com/CesiumGS/cesium-unity/issues/1) - The very first issue in the cesium-unity repo documents the transition from Jackson Dunstall's UnityNativeScripting to Reinterop. Useful to understand what's different betwen them, and why.
* [Interop with Native Libraries (from the Mono Project)](https://www.mono-project.com/docs/advanced/pinvoke/) - Probably the best resource for understanding how to call between C# and C using P/Invoke (which Reinterop is built on). Some of it is Mono-specific, which is fine in the context of Unity since it uses Mono. Most of it applies to Microsoft .NET and CoreCLR, too.
* [x64 Calling Convention](https://learn.microsoft.com/en-us/cpp/build/x64-calling-convention) - If you ever find yourself in the unfortunate position of needing to debug a crash or other problem at the C# / C++ boundary, you'll probably need to look at the disassembly. And when you're doing that, it's helpful to understand how parameters are passed to and returned from functions at the lowest levels. This is processor and sometimes operating system specific. This is the documentation for Intel/AMD x64 processors, which I believe should be applicable on both Windows and Linux.
* Roslyn Resources - Roslyn is the name of the C# compiler. Reinterop is a "source generator" that plugs into Roslyn.
  * [Source Generators](https://github.com/dotnet/roslyn/blob/main/docs/features/source-generators.md) - The most official (to my knowledge) documentation on Roslyn source generators (but it's pretty basic).
  * [Source Generators Cookbook](https://github.com/dotnet/roslyn/blob/main/docs/features/source-generators.cookbook.md) - How to do various things in a Roslyn source generator, direct from the Roslyn developers.
  * [Incremental Generators](https://github.com/dotnet/roslyn/blob/main/docs/features/incremental-generators.md) - Roslyn's documentation on incremental generators, which again we're not currently using.
  * [Incremental Roslyn Source Generators](https://www.thinktecture.com/en/net/roslyn-source-generators-introduction/) - Intro to the C# compiler (Roslyn) feature that we're using as the "hook" to generate the bindings layer. Note that Reinterop isn't an "incremental" source generator anymore (though there are still remnants in the code base) because Unity 2021.3, at least, doesn't support the incremental APIs.
  * [Working with types in a Roslyn analyzer](https://www.meziantou.net/working-with-types-in-a-roslyn-analyzer.htm) - A useful cookbook for doing various things with types inside a Roslyn analyzer, most of which is applicable to a Roslyn source generator, too.
